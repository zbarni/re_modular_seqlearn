...
experiment_label = 'alt_model_local_inh'
...
ParameterRange = {
    'n_col': [2],
    'w_IE_L23_L5': [0.2, 0.21],
    'T': np.arange(1, 4, 1)
}


# ################################
def build_parameters(n_col, w_IE_L23_L5, T):
    # system_params = set_system_parameters(cluster=system_label, nodes=1, ppn=6, mem=512000)
    system_params = set_system_parameters(cluster=system_label, nodes=1, ppn=80, mem=200000)
    ...
    connection_params = {
        ...
        'model2': True  # alternative wiring
    }
    ...
    input_params = {
        ...
        'misc': {
            ...
            'trial_isi': 200.,
            'stim_inh_pop': False  # whether to stimulate the inhibitory populations in L5
        },
    ...
    recording_params = {
        ...
        'record_spikes_pop': [naming.get_pop_name(p, col_id)
                              for col_id in range(n_col) for p in ['timer_exc', 'messenger_exc',
                                                                   'timer_inh', 'messenger_inh']],
    }
