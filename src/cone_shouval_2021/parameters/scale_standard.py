...
experiment_label = 'scale_standard'
...
def build_parameters(T):
    ...
    delay = 1.
    density = 0.26
    wscale = (np.sqrt(100) * density) / (np.sqrt(N) * density)

    # naming convention is TGT_SRC, i.e., Target <- Source
    rescaled_weights = {
        'w_EE_L5_rec': 0.07 * wscale,
        'w_EE_L5_L23_cross': 0.0002 * wscale,
        'w_EE_L23_L5': 0.2 * wscale,
        'w_EI_L23_L5':  -70 * wscale,
        'w_EI_L23_L23_cross': -100. *wscale,
        'w_EI_L5_L5_cross': -100. * wscale,
        'w_IE_L5_rec': 0.2 * wscale,        # must be scaled because rate I_L5 is too high otherwise
        'w_IE_L23_rec': 1. * wscale,
    }
    learn_rate_ff = 0.25 * 1e3 / 12.5 / 10.  # for N = 400, to allow more time
