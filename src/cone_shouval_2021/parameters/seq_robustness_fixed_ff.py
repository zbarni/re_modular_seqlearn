"""
Robustness to variability in the cross-columnar inhibitory weights (4x700ms).

Used to produce figures 3B (right panels);
"""
...
experiment_label = 'robustness_fixed_ff_4x700'
...
# ################################
ParameterRange = {
    'w_EI_L5_cross': [-20., 0., 20.],
    'w_EI_L23_cross': [-20., 0., 20.],
    'T': np.arange(1, 11, 1)
}
n_col = 4

# ################################
def build_parameters(w_EI_L5_cross, w_EI_L23_cross, T):
    ...
    connection_params = {
        'conns': {
            ...
            # cross-columnar I->E
            'w_EI_L23_L23_cross': {
                'weight': -100.0 - 100. * w_EI_L23_cross / 100.,
                'density': density,
                'delay': delay,
                'distribution': (np.random.normal, {'loc': 0., 'scale': 0.1})
            },
            'w_EI_L5_L5_cross': {
                'weight': -100.0 - 100. * w_EI_L5_cross / 100.,
                'density': density,
                'delay': delay,
                'distribution': (np.random.normal, {'loc': 0., 'scale': 0.1})
            },
