"""
All-to-all FF connections, with background noise but higher Hebbian threshold
"""
...
experiment_label = 'seq_replay_4x700_HebbTh'
# experiment_label = 'seq_replay_set=org_HebbTh'
...
# ################################
ParameterRange = {
    # 'th_rec': np.arange(0.0, 0.021, 0.01),
    'th_rec': [0.01],
    'th_ff': np.arange(0.0, 0.031, 0.01),
    'T': np.arange(1, 4, 1)
}


# ################################
def build_parameters(th_rec, th_ff, T):
# def build_parameters(th_ff, T):
    # system_params = set_system_parameters(cluster=system_label, nodes=1, ppn=6, mem=8000)
    system_params = set_system_parameters(cluster=system_label, nodes=1, ppn=64, mem=200000)
    ...
    input_params = {
        ...
        'misc': {
            ...
            # 'token_duration': [500., 900., 700., 1200.][:n_col] + [1000.],
            'trial_isi': 200.,
